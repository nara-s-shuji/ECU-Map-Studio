...
function switchTab(tabId) {
    ...
    document.getElementById('info-filename').innerText = (typeof currentFileName !== 'undefined' ? currentFileName : 'No File') + ' (debug_113)';
    const headerDisplay = document.querySelector('.file-view-title');
    if (headerDisplay) headerDisplay.innerHTML = `ECU Map Studio <span style="font-size:10px; color:#888; margin-left:5px;">(debug_113)</span>`;

    // Ensure settings menu title also updates
    const menuTitle = document.querySelector('#settings-menu .file-view-title');
    if (menuTitle) menuTitle.innerHTML = `ECU Map Studio <span style="font-size:10px; color:#888; margin-left:5px;">(debug_113)</span>`;
